package org.example.leavemanagement.helpers;
...
/**
 * Enhanced UserApiClient with Keycloak integration and Leave Management cleanup
...
public class UserApiClient {
    ...
    private final String leaveServiceUrl = System.getProperty("leave.service.url", "http://localhost:8085");
    ...
    // ========== LEAVE MANAGEMENT CLEANUP METHODS ==========

    /**
     * Delete all leave requests for a specific user
     * CRITICAL: This ensures complete cleanup after tests
     * @param username The username whose leave requests should be deleted
     */
    public void deleteLeaveRequestsForUser(String username) {
        try {
            System.out.println("Attempting to delete leave requests for user: " + username);

            // Get user ID first
            String userId = getUserIdByUsername(username);
            if (userId == null) {
                System.out.println("User " + username + " not found in application, skipping leave cleanup");
                return;
            }

            ensureAdminToken();

            // Try different possible endpoints for deleting user leaves
            String[] deleteEndpoints = {
                    "/api/leaves/user/" + userId,
                    "/api/conge/user/" + userId,
            ...
            for (String endpoint : deleteEndpoints) {
                try {
                    HttpRequest request = HttpRequest.newBuilder()
                            .uri(URI.create(leaveServiceUrl + endpoint))
                    ...
                    if (response.statusCode() == 200 || response.statusCode() == 204) {
                        System.out.println("Successfully deleted leave requests for user: " + username + " using endpoint: " + endpoint);
                        deleted = true;
                        break;
                    } else if (response.statusCode() == 404) {
                        System.out.println("No leave requests found for user: " + username + " at endpoint: " + endpoint);
                        deleted = true;
                        break;
                    }
                } catch (Exception e) {
            ...
            if (!deleted) {
                System.out.println("WARNING: Could not delete leave requests for " + username + " - endpoint may not exist");
            }

        } catch (Exception e) {
            System.err.println("Error deleting leave requests for " + username + ": " + e.getMessage());
            // Don't throw - we want cleanup to continue even if one user fails
        }
    }

    /**
     * Delete all leave requests (nuclear option for cleanup)
        ...
        try {
            ...
            for (String endpoint : deleteEndpoints) {
                try {
                    ...
                    if (response.statusCode() == 200 || response.statusCode() == 204) {
                        System.out.println("Successfully deleted all leave requests using endpoint: " + endpoint);
                        return;
                    }
                } catch (Exception e) {
                    System.out.println("Endpoint " + endpoint + " failed: " + e.getMessage());
            ...
            System.out.println("WARNING: Could not delete all leave requests - no working endpoint found");

        } catch (Exception e) {
            System.err.println("Error deleting all leave requests: " + e.getMessage());
        }
    }

    /**
     * Get leave requests for a user (for verification)
     */
    public List<Map<String, Object>> getLeaveRequestsForUser(String username) {
        try {
            String userId = getUserIdByUsername(username);
            if (userId == null) {
                return List.of();
            ...
            for (String endpoint : getEndpoints) {
                try {
                    ...
                    if (response.statusCode() == 200) {
                        ...
                        List<Map<String, Object>> leaves = objectMapper.readValue(response.body(), List.class);
                        return leaves;
                    }
                } catch (Exception e) {
