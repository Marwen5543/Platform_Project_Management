...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  pendingTeamLeavesCount = 0; // Added for leave notifications
  ...
  constructor(
    ...
    private leaveService: LeaveService, // Added
  ...
  private startDocumentCheck(): void {
    if (this.canRequestAttestations()) {
      this.documentCheckSubscription = interval(300000).pipe(
        startWith(0),
        switchMap(() => this.documentService.getDocumentHistory())
      ).subscribe({
        next: (documents) => {
          const newReadyCount = documents.filter(doc =>
  ...
  private startLeaveCheck(): void {
    if (this.hasManagerRole()) {
      this.leaveCheckSubscription = interval(300000).pipe(
        startWith(0),
  ...
  toggleMobileDropdown(dropdown: 'conges' | 'documents' | 'doc-management' | 'admin'): void {
    ...
    this.isDocManagementDropdownOpen = dropdown === 'doc-management' ? !this.isDocManagementDropdownOpen : false;
  ...
  canRequestAttestations(): boolean {
    return [UserRole.EMPLOYEE, UserRole.MANAGER, UserRole.ADMIN].includes(this.user?.role) &&
           ![UserRole.HR, UserRole.SUPER_ADMIN].includes(this.user?.role);
  }
