// File: user-management-service/src/test/scala/company/user_management_service/simulations/UserManagementSimulation.scala
...
class UserManagementSimulation extends Simulation {
  ...
  // --- Data Feeder ---
  val allUsersFeeder = csv("users.csv").circular
  ...
  val mockLogin = exec(
    http("Attempt Login for #{username}")
      ...
      .check(status.in(200, 401, 403, 404, 500)) // Accept any status, just send the request
  ).exec(session => {
  ...
  val employeeScenario = scenario("Employee User Journey")
    .feed(allUsersFeeder)
    .doIf(session => session("role").as[String] == "EMPLOYEE") {
      exec(mockLogin)
  ...
  val adminScenario = scenario("Admin User Journey")
    .feed(allUsersFeeder)
    .doIf(session => session("role").as[String] == "ADMIN") {
      exec(mockLogin)
        .pause(1.second)
        .exec(
  ...
  val superAdminScenario = scenario("Super Admin Full Access Journey")
    .feed(allUsersFeeder)
    .doIf(session => session("role").as[String] == "SUPER_ADMIN") {
      exec(mockLogin)
        .pause(1.second)
        .exec(
          http("Super Admin Deletes a User (Simulated)")
            // *** CORRECTED PATH ***
  ...
  // A simple health check scenario that we know works
  ...
  // --- LOAD SETUP ---
  setUp(
    // We run all scenarios. The doIf will filter them.
    ...
    // Also inject some simple health checks
